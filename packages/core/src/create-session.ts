...
import { TOOL_USE_PROMPT } from "./tools/tool-use-prompt.js";
...
const SUB_AGENT_GUIDANCE = '- For investigation or planning subtasks, consider delegating to a sub-agent via the SubAgent tool (type: "explore" to investigate the codebase or web, type: "plan" to produce an implementation plan) and continue the work yourself based on its report. Sub-agents are read-only and cannot write or edit files; perform any edits yourself.';
