export const TOOL_USE_PROMPT = [
  ...
  "- Scale planning: when a task involves many independent items (queries, reads, searches), estimate the count up front and pick a strategy: a few (<=10) — do them directly; tens — spread over a few turns with several calls per turn; dozens to hundreds — delegate chunks via SubAgent (each chunk runs its own loop and budget, not yours).",
